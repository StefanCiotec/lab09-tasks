...
static int minfs_readdir(struct file *filp, struct dir_context *ctx)
{
	...
	struct minfs_inode_info *mii = list_entry(inode, struct minfs_inode_info, vfs_inode);
...
static struct inode *minfs_new_inode(struct inode *dir)
{
	...
	sbi->sbh->b_data = &sbi->imap;
	idx = find_first_zero_bit(sbi->sbh->b_data, MINFS_NUM_ENTRIES);
	/* TODO 5: mark superblock buffer head as dirty */
	if (!sbi->sbh || (idx > MINFS_NUM_ENTRIES)) {
		iput(inode);
		return NULL;
	}
	__set_bit(idx, sbi->sbh->b_data);
...
static int minfs_add_link(struct dentry *dentry, struct inode *inode)
{
	...
	struct minfs_inode_info *mii = container_of(dir,
			struct minfs_inode_info, vfs_inode);
	struct minfs_dir_entry *de = NULL;
	int i;
	int err = 0;
	...
	if (de == NULL)
		return NULL;
	de->ino = i;
	memcpy(de->name, name, MINFS_NAME_LEN);
